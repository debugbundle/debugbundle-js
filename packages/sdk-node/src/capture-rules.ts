...
import type {
  NodeCaptureRule,
  NodeCaptureRuleAction,
  NodeCaptureRuleEvaluationResult,
  NodeCaptureRuleMatcher,
  NodeCaptureRuleRuntime,
  NodeCaptureRuleSampleEventClass,
  NodeCaptureRuleUrlMatcher
} from "./types.js";

type NodeCaptureRuleEvaluationUrl = {
  host?: string;
  path: string;
};

type NodeCaptureRuleEvaluationContext = {
  project_id: string;
  event_id: string;
  ...
  error_name?: string;
  message?: string;
  request_url?: NodeCaptureRuleEvaluationUrl;
  status_code?: number;
  fingerprint?: {
    version: string;
    value: string;
...
function normalizeRuntime(value: string | null | undefined): NodeCaptureRuleRuntime {
...
function normalizeEvaluationUrl(value: string | null | undefined): {
  url?: NodeCaptureRuleEvaluationUrl;
  first_party?: boolean;
} {
  ...
  try {
    const parsed = new URL(trimmed, "http://debugbundle.local");
    if (parsed.protocol === "http:" || parsed.protocol === "https:") {
      return {
        ...
        first_party: parsed.hostname === "debugbundle.local"
      };
    }
  } catch {
    return {};
...
function parseUrlMatcher(value: unknown): NodeCaptureRuleUrlMatcher | undefined {
...
function parseMatcher(value: unknown): NodeCaptureRuleMatcher | null {
  const record = asRecord(value);
  if (record === null) {
    return null;
  }
...
function parseCaptureRule(value: unknown): NodeCaptureRule | null {
  const record = asRecord(value);
  if (record === null) {
    return null;
  }
  ...
  const action: NodeCaptureRuleAction | null =
  ...
  const sampleEventClass: NodeCaptureRuleSampleEventClass | null =
...
export function parseRemoteCaptureRulesPayload(payload: unknown): NodeCaptureRule[] {
  ...
  return record["capture_rules"]
    ...
    .filter((rule): rule is NodeCaptureRule => rule !== null);
}

function buildEvaluationContext(projectId: string, event: EventEnvelope): NodeCaptureRuleEvaluationContext {
  const base: NodeCaptureRuleEvaluationContext = {
    project_id: projectId,
  ...
  if (event.event_type === "backend_exception") {
    const requestUrl = normalizeEvaluationUrl(event.payload.request.path);
    return {
      ...base,
      ...
      status_code: event.payload.response.status_code
    };
  }
...
function matchesUrlMatcher(
  matcher: NodeCaptureRuleUrlMatcher | undefined,
  value: NodeCaptureRuleEvaluationUrl | undefined
): boolean {
...
function matchesRule(rule: NodeCaptureRule, context: NodeCaptureRuleEvaluationContext): boolean {
  ...
  if (
    matcher.message_contains !== undefined &&
    (context.message === undefined || !context.message.includes(matcher.message_contains))
  ) {
    return false;
  }
  ...
  if (
    matcher.status_codes !== undefined &&
    (context.status_code === undefined || !matcher.status_codes.includes(context.status_code))
  ) {
    return false;
  }
  if (
...
function getSpecificityScore(rule: NodeCaptureRule): number {
  const matcher = rule.matcher;
  let score = 0;
  ...
  if (matcher.request_url?.host_suffix !== undefined) {
    score += 90;
  }
  if (matcher.request_url?.path_prefix !== undefined) {
    score += 80;
  }
...
function compareRules(left: NodeCaptureRule, right: NodeCaptureRule): number {
...
export function evaluateNodeCaptureRulesForEvent(
  rules: readonly NodeCaptureRule[],
  projectId: string,
  event: EventEnvelope,
  now: string
): NodeCaptureRuleEvaluationResult | null {
